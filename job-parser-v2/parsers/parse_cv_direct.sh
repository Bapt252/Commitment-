...
if [ "$REFRESH" = true ]; then
  ...
  endpoint="$API_URL/parse-cv/"
  response=$(curl -s -X POST -F "file=@$PDF_PATH" -F "force_refresh=true" "$endpoint")
else
  endpoint="$API_URL/parse-cv/"
  response=$(curl -s -X POST -F "file=@$PDF_PATH" "$endpoint")
fi
...
# Affichage de la réponse directe (car l'API renvoie directement le résultat)
