"""
Nexten SmartMatch - Système de matching bidirectionnel avancé
-------------------------------------------------------------
Système de matching bidirectionnel prenant en compte le temps de trajet via Google Maps,
l'analyse sémantique des compétences et générant des insights détaillés.
...
class SmartMatcher:
    ...
    - Calcul du temps de trajet via Google Maps API
    - Préférences des candidats et exigences des employeurs
    - Générations d'insights détaillés sur chaque match
    """
    ...
    def _build_skill_synonyms(self) -> Dict[str, List[str]]:
        ...
        # Ajouter des synonymes à partir de WordNet
    ...
    def calculate_travel_time(self, origin: str, destination: str) -> int:
        ...
        Returns:
            int: Temps de trajet en minutes, ou -1 en cas d'erreur
        """
        if not self.api_key:
            logger.warning("Calcul de distance impossible : pas de clé API Google Maps")
            return -1
    ...
    def calculate_location_match(self, candidate: Dict[str, Any], job: Dict[str, Any]) -> float:
        ...
        # Si le calcul a échoué, retourner un score neutre
        if travel_time < 0:
            return 0.5
    ...
    def calculate_experience_match(self, candidate: Dict[str, Any], job: Dict[str, Any]) -> float:
        ...
        max_years = job.get("max_years_of_experience", 100)
    ...
    def calculate_preference_match(self, candidate: Dict[str, Any], job: Dict[str, Any]) -> float:
        ...
        if "salary_expectation" in candidate and "salary_range" in job:
            ...
            job_min_salary = job["salary_range"]["min"]
            job_max_salary = job["salary_range"]["max"]
    ...
    def generate_insights(
        ...
        if "salary_expectation" in candidate and "salary_range" in job:
            ...
            if candidate_salary > job_max_salary:
                insights.append({
                    "type": "salary_mismatch",
                    "message": "Attentes salariales supérieures au budget du poste",
                    "score": min(0.5, job_max_salary / candidate_salary),
                    "category": "mismatch"
                })
