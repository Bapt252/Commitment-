...
def check_fastapi_installed():
    """Vérifie que FastAPI est correctement installé"""
    try:
        import fastapi
        import uvicorn
        
        logger.info(f"FastAPI version {fastapi.__version__} installée correctement")
        logger.info(f"Uvicorn version {uvicorn.__version__} installé correctement")
        
        return True
    except ImportError as e:
        logger.error(f"FastAPI ou Uvicorn n'est pas installé correctement: {e}")
        return False

if __name__ == "__main__":
    logger.info("Démarrage du préchargement des modèles pour le service matching...")
    
    # Exécuter tous les préchargements
    results = [
        ("Sentence Transformers", preload_sentence_transformers()),
        ("NLTK", preload_nltk_data()),
        ("FastAPI", check_fastapi_installed())
    ]
