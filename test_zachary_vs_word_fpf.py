#!/usr/bin/env python3
"""
🏆 Test spécialisé : Zachary vs Fiches de Poste Word (TEST FPF)
Version adaptée pour traiter les fichiers .doc et .docx
"""
...
def find_word_files(directory):
    """Trouve tous les fichiers Word dans le dossier"""
    word_files = []
    
    # Extensions Word supportées
    word_extensions = ['*.doc', '*.docx']
    
    for extension in word_extensions:
        word_files.extend(list(directory.glob(extension)))
    
    # Chercher aussi dans les sous-dossiers
    for subdirectory in directory.iterdir():
        if subdirectory.is_dir():
            for extension in word_extensions:
                word_files.extend(list(subdirectory.glob(extension)))
...
def parse_word_job(job_file):
    """Parser un fichier Word via l'API Job Parser"""
    try:
        # Déterminer le type MIME selon l'extension
        if job_file.suffix.lower() == '.docx':
            mime_type = 'application/vnd.openxmlformats-officedocument.wordprocessingml.document'
        else:
            mime_type = 'application/msword'
        
        with open(job_file, 'rb') as f:
            files = {'file': (job_file.name, f, mime_type)}
            response = requests.post(
        ...
        if response.status_code == 200:
            ...
            print(f"   ❌ Erreur parsing Job Word: {response.status_code}")
            print(f"   📄 Réponse: {response.text[:200]}...")
            return None
            
    except Exception as e:
        print(f"   ❌ Exception parsing Job Word: {e}")
        return None
...
def test_zachary_vs_word_fpf():
    """Test principal : Zachary vs Fiches Word TEST FPF"""
    print("🏆 TEST SPÉCIALISÉ : ZACHARY vs TEST FPF (WORD)")
    print("=" * 60)
    print("🎯 Zachary a obtenu 94% sur les tests précédents")
    print("📄 Testons-le maintenant sur les fiches de poste Word !")
    print()
    ...
    # Lister les fiches de poste Word
    print("\\n🔍 Recherche des fichiers Word (.doc, .docx)...")
    word_files = find_word_files(fpf_dir)
    
    if not word_files:
        print(f"❌ Aucun fichier Word trouvé dans {fpf_dir}")
        print("💡 Extensions recherchées: .doc, .docx")
        
        # Afficher ce qui est trouvé
        all_files = list(fpf_dir.iterdir())
        if all_files:
            print(f"\\n📋 Fichiers présents dans le dossier:")
            for file in all_files[:10]:  # Limiter à 10 fichiers
                if file.is_file():
                    print(f"   - {file.name}")
        
        return False
    
    print(f"✅ {len(word_files)} fichier(s) Word trouvé(s) :")
    for word_file in word_files:
        size = word_file.stat().st_size / 1024
        print(f"   - {word_file.name} ({size:.1f} KB)")
    ...
    # Tester contre chaque fiche de poste Word
    print(f"\\n🚀 DÉBUT DES TESTS CONTRE {len(word_files)} FICHES WORD")
    print("=" * 60)
    ...
    for i, job_file in enumerate(word_files, 1):
        print(f"\\n💼 Test {i}/{len(word_files)}: {job_file.name[:50]}...")
        
        # Parser la fiche de poste Word
        job_data = parse_word_job(job_file)
        if not job_data:
            print("   ❌ Échec parsing fiche de poste Word")
            parsing_errors.append(job_file.name)
            continue
        
        # Vérifier que les données sont valides
        if not job_data or (not job_data.get('missions') and not job_data.get('requirements')):
            print("   ⚠️ Fiche de poste vide ou mal parsée")
            parsing_errors.append(f"{job_file.name} (données vides)")
            continue
        
        print("   ✅ Fiche de poste parsée avec succès")
    ...
    print("\\n" + "=" * 60)
    print("📊 RÉSULTATS FINAUX - ZACHARY vs TEST FPF (WORD)")
    print("=" * 60)
    
    if parsing_errors:
        print(f"⚠️ Erreurs de parsing sur {len(parsing_errors)} fichier(s):")
    ...
    if results:
        ...
        print(f"🎯 Fichiers Word traités: {len(word_files)}")
        print(f"✅ Tests réussis: {len(results)}")
        print(f"❌ Erreurs de parsing: {len(parsing_errors)}")
        ...
        report_file = f"zachary_vs_word_fpf_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        ...
        print("💡 Vérifiez que le Job Parser V2 supporte les fichiers Word")
        return False
...
def main():
    ...
    # Note importante sur les fichiers Word
    print("📝 NOTE: Ce script traite les fichiers Word (.doc/.docx)")
    print("🔧 Si le Job Parser ne supporte pas Word, il faudra les convertir en PDF")
    print()
    
    # Lancer le test
    return test_zachary_vs_word_fpf()
