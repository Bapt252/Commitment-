...
class MetricsCollector:
    ...
    async def initialize(self):
        ...
        try:
            ...
            if self.prometheus_enabled:
                ...
                logger.info(f"✅ Serveur Prometheus démarré sur port {settings.prometheus_port}")
    ...
    async def record_error(self, error_type: str, endpoint: str):
        ...
        if self.redis_client:
            try:
                key = f"{settings.cache_prefix}:errors"
                await self.redis_client.lpush(key, json.dumps(metric))
                await self.redis_client.expire(key, settings.metrics_retention_hours * 3600)
            except Exception as e:
    ...
    async def _store_metric_in_redis(self, metric_type: str, metric: MatchingMetric):
        ...
        key = f"{settings.cache_prefix}:metrics:{metric_type}"
        data = asdict(metric)
    ...
    async def _get_historical_metrics(self) -> Dict[str, Any]:
        ...
        key = f"{settings.cache_prefix}:metrics:matching_requests"
    ...
    async def _cleanup_old_metrics(self):
        ...
        while True:
            try:
                await asyncio.sleep(settings.metrics_cleanup_interval)
