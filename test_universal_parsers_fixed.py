...
"""
🧪 SuperSmartMatch V2.1 - Tests Universal Parsers (FIXED)
...
try:
    ...
    HAS_DOCX = False
    print("⚠️ python-docx non installé, tests DOCX désactivés")
...
try:
    ...
    HAS_PIL = False
    print("⚠️ Pillow non installé, tests images désactivés")
...
"""
        ...
        # 3. Fichier HTML (.html) - FIX: séparation des backslashes
        newline_to_br = cv_content.replace('\n', '<br>\n')
...
"""
        ...
        # 4. Fichier RTF (.rtf) - FIX: séparation des backslashes
        newline_to_par = cv_content.replace('\n', '\\par\n')
        rtf_content = f"""{{\\rtf1\\ansi\\deff0
{{\\fonttbl{{\\f0 Times New Roman;}}}}
\\f0\\fs24 {newline_to_par}
}}"""
    ...
    def run_comprehensive_tests(self) -> Dict:
        ...
        self._generate_test_summary(results, test_files)
        
        return results
    
    def _generate_test_summary(self, results: Dict, test_files: Dict):
...
def main():
    ...
    logger.info("🧪 SuperSmartMatch V2.1 - Tests Universal Parsers (FIXED)")
    logger.info("=" * 60)
