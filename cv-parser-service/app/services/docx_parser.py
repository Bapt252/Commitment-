...
def parse_docx(file_path):
    """
    Parse a DOCX CV to extract structured information
...
def extract_cv_data(text):
    ...
    # Extract languages
    languages_section = re.search(r'(?:Languages|Langues)[:\s]*(.*?)(?:\n\n|\Z)', text, re.IGNORECASE | re.DOTALL)
    if languages_section:
        languages_text = languages_section.group(1)
        # Split languages by commas or newlines
        languages = re.split(r'[,\n]+', languages_text)
        
        for language in languages:
            language = language.strip()
            if language:
                # Try to extract language and proficiency level
                level_match = re.search(r'([A-Za-z\s]+)\s*[-:]\s*([A-Za-z0-9\s/]+)', language)
                if level_match:
                    cv_data["languages"].append({
                        "language": level_match.group(1).strip(),
