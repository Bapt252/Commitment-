# CV Parser Service - Routes directes pour le parsing de CV
...
from app.utils.validation import validate_cv_file
...
@direct_router.post("/parse-cv", status_code=status.HTTP_200_OK)
async def parse_cv_direct(
    ...
    """Parse un CV directement sans passer par la file d'attente Redis"""
    ...
    try:
        ...
        try:
            # 4. Parser le CV directement
            start_time = time.time()
            result = parse_cv(temp_path, file_extension)
            ...
            logger.info(f"CV parsé directement en {processing_time:.2f}s: {file.filename}")
        ...
        # Erreurs inattendues
        logger.error(f"Erreur lors du parsing direct du CV: {str(e)}")
        raise HTTPException(
